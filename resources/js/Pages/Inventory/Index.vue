                ...
                <table class="w-full text-left border-collapse min-w-[1000px]">
                    ...
                    <tbody class="text-sm text-gray-600 divide-y divide-gray-50">
                        <tr v-for="(item, index) in spareParts.data" :key="item.id" class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-3 py-3 sm:py-4 text-xs sm:text-[13px] whitespace-nowrap">{{ item.location ? item.location.name : '-' }}</td>
                            <td class="px-3 py-3 sm:py-4 text-center whitespace-nowrap">
                            ...
                            <td class="px-3 py-3 sm:py-4 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-2">
                                    <button @click="openDetailModal(item)" title="Lihat Detail" class="text-[#312e81] font-semibold text-xs hover:bg-indigo-50 p-1.5 sm:p-2 rounded-lg transition-colors border border-[#312e81]/20">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </button>
                                    <button @click="openEditModal(item)" title="Edit Barang" class="text-gray-600 font-semibold text-xs hover:bg-gray-100 p-1.5 sm:p-2 rounded-lg transition-colors border border-gray-200">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </button>
                                    <button @click="deleteItem(item)" title="Hapus Barang" class="text-red-500 font-semibold text-xs hover:bg-red-50 p-1.5 sm:p-2 rounded-lg transition-colors border border-red-200">
                    ...
                    <form @submit.prevent="submitForm">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <div class="flex items-start gap-4">
                                    <!-- Image Preview Box -->
                                    <div class="relative w-24 h-24 sm:w-32 sm:h-32 flex-shrink-0 bg-gray-50 border border-gray-200 rounded-lg overflow-hidden flex items-center justify-center">
                                        <img v-if="imagePreview" :src="imagePreview" class="w-full h-full object-cover" />
                                        <div v-else class="text-gray-400 flex flex-col items-center">
                                            <svg class="w-8 h-8 sm:w-10 sm:h-10 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2-2v12a2 2 0 002 2z"></path></svg>
                                    ...
                                    <div class="flex-1">
                                        <div class="relative inline-block w-full">
                                            <input type="file" @change="handleManualImageUpload" accept="image/*" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-[#312e81] hover:file:bg-indigo-100 cursor-pointer transition-colors">
                                        </div>
                                        <p class="text-[10px] text-gray-400 mt-2">*Maksimal 5MB. Kosongkan jika tidak ingin mengubah gambar.</p>
